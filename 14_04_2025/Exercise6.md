...
#get data only from 2007
df_2007 = df_gapminder[df_gapminder['year'] == 2007]

#normal graph with count
sns.histplot(data=df_2007, x='population', stat='count')
plt.title('Distribution of Population in 2007 (Count)')
plt.xlabel('Population')
plt.ylabel('Count')
plt.show()


#bonus graph with probability
...
#observations:
#graph is right skewed, can substitute one variable into a logarithm to make it more linear
```
...
#get data only for americas
df_americas = df_gapminder[df_gapminder['continent'] == 'Americas']

sns.lineplot(data=df_americas, x='year', y='gdp_cap')
plt.title('Average GDP per capita in Americas')
plt.xlabel('Year')
