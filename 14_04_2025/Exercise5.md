...
#its showing that the most frequent attack values are around 75-100, distribution is skewed to the right - most pokemon have lower attack values
```
...
```python
### Your code here
#no, this is qualitative - a barplot would be better instead
```
...
```
    ...
    <matplotlib.patches.FancyArrow at 0x1b7b6c06270>
...
#seems normal, outliers are on the right where the arrow is
```




    <matplotlib.patches.FancyArrow at 0x1b7b8dd8b90>




    
![png](Exercise5_files/Exercise5_39_1.png)
...
#this is slightly right skewed - tail is extended to the right. no outliers
```


    
![png](Exercise5_files/Exercise5_41_0.png)
...
Lets try to group by our no. of attacks by pokemon types looking at many histograms at a time
...
#yes, they are. when x increases, y increases as well
```
...
y = -x * 2 + np.random.normal(loc = 0, scale = 2, size = 100)
plt.scatter(x, y, alpha = .6)
#yes, they are. when x increases, y decreases
```
...
![png](Exercise5_files/Exercise5_63_0.png)
    


# Scatterplots with pyplot express

With pyplot we can express scatterplots even further - we can create bubble plots
...
#legendary pokemons usually have a higher attack value
#the missing part might be the title of the plot
```


    
![png](Exercise5_files/Exercise5_74_0.png)
...
```python
fig = px.bar(long_df,  x="nation",  facet_row='medal', color='nation')
fig.show()
```
